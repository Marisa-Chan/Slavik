#include "simix.h"
#include <algorithm>
#include <SDL2/SDL.h>

namespace Simix
{

/*from SDL*/

static const uint8_t mix8[] = {
    
};

/* The volume ranges from 0 - 128 */
#define ADJUST_VOLUME(s, v)     ((s) = ((s) * (v)) / SDL_MIX_MAXVOLUME)
#define ADJUST_VOLUME_U8(s, v)  ((s) = ((((s) - 128) * (v)) / SDL_MIX_MAXVOLUME) + 128)
#define ADJUST_VOLUME_U16(s, v) ((s) = ((((s) - 32768) * (v)) / SDL_MIX_MAXVOLUME) + 32768)

#define GETFINTPOS(v) (v >> 8)
#define GETFRACPOS(v) (v & 0xFF)


//Hacky internal func
inline bool CheckEnd(MixChannel *chn, uint32_t *pos, uint32_t *fpos, uint32_t *stpos, uint32_t *endpos, uint32_t realEnd)
{
    if (*pos < *endpos)
        return false;

    if (chn->loop == 0)
    {
        chn->active = false;
        return true;
    }

    *fpos = chn->smplpos = GETFRACPOS(*fpos);

    if (chn->loopStart > 0)
        *stpos = chn->loopStart;
    else
        *stpos = 0;

    *pos = *stpos;

    if (chn->loop > 0)
    {
        chn->loop--;
        if (chn->loop == 0 && chn->afterloop)
            *endpos = realEnd;
    }

    return false;
}

void Mixer::MixBufferChannel(Uint8 *dst, int32_t dstlen, MixChannel *chn)
{
    MixBuffer *buf = chn->data;

    const float fspeedfactor = ((float)buf->freq / (float)_spec.freq) * chn->pitch;
    const uint32_t speedfactor = fspeedfactor * 256.0f;

    const int32_t lvlm = chn->pan > 0 ? chn->volume *  chn->pan / 128 : chn->volume;
    const int32_t rvlm = chn->pan < 0 ? chn->volume * -chn->pan / 128 : chn->volume;

    const int32_t SampleSize = ( SDL_AUDIO_BITSIZE(_spec.format) / 8 ) * buf->channels;
    const uint32_t srclen = buf->size & ~(SampleSize - 1);

    uint32_t endpos = srclen ;

    if (chn->loop != 0 && chn->loopEnd > 0)
        endpos = chn->loopEnd;

    uint32_t fpos = chn->smplpos;
    uint32_t stpos = chn->pos;    

    if (buf->channels == 2)
    {
        switch(_spec.format)
        {
            case AUDIO_U8:
            {
                const uint8_t *src = (uint8_t *)buf->data.data();
                uint8_t lsmpl, rsmpl;
                
                dstlen /= 2;

                while (dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = src[ pos / SampleSize];
                    rsmpl = src[ pos / SampleSize + 1 ];
                    ADJUST_VOLUME_U8(rsmpl, rvlm);
                    ADJUST_VOLUME_U8(lsmpl, lvlm);

                    *dst = mix8[*dst + lsmpl];
                    ++dst;
                    *dst = mix8[*dst + rsmpl];
                    ++dst;

                    fpos += speedfactor;
                }
                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            }
            break;

            case AUDIO_S8:
            {
                const int8_t *src = (int8_t *)buf->data.data();
                int8_t *dst8 = (int8_t *)dst;
                int8_t lsmpl, rsmpl;
                
                dstlen /= 2;

                while (dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = src[ pos / SampleSize ];
                    rsmpl = src[ pos / SampleSize + 1 ];
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst8 = CLAMP<int16_t>((int16_t)*dst8 + (int16_t)lsmpl, SDL_MIN_SINT8, SDL_MAX_SINT8);
                    ++dst8;
                    *dst8 = CLAMP<int16_t>((int16_t)*dst8 + (int16_t)rsmpl, SDL_MIN_SINT8, SDL_MAX_SINT8);
                    ++dst8;
                    
                    fpos += speedfactor;
                }
                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            }
            break;

            case AUDIO_S16LSB:
            {
                const int16_t *src = (int16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                int16_t *dst16 = (int16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapLE16(src[pos / SampleSize]);
                    rsmpl = SDL_SwapLE16(src[pos / SampleSize + 1]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>((int32_t)SDL_SwapLE16(*dst16) + lsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;
                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>((int32_t)SDL_SwapLE16(*dst16) + rsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_S16MSB:
            {
                const int16_t *src = (int16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                int16_t *dst16 = (int16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapBE16(src[pos / SampleSize]);
                    rsmpl = SDL_SwapBE16(src[pos / SampleSize + 1]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>((int32_t)SDL_SwapBE16(*dst16) + lsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;
                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>((int32_t)SDL_SwapBE16(*dst16) + rsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_U16LSB:
            {
                const uint16_t *src = (uint16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                uint16_t *dst16 = (uint16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapLE16(src[pos / SampleSize]);
                    rsmpl = SDL_SwapLE16(src[pos / SampleSize + 1]);
                    ADJUST_VOLUME_U16(rsmpl, rvlm);
                    ADJUST_VOLUME_U16(lsmpl, lvlm);

                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>(((int32_t)SDL_SwapLE16(*dst16) + lsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768);
                    ++dst16;
                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>(((int32_t)SDL_SwapLE16(*dst16) + rsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768 );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_U16MSB:
            {
                const uint16_t *src = (uint16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                uint16_t *dst16 = (uint16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapBE16(src[pos / SampleSize]);
                    rsmpl = SDL_SwapBE16(src[pos / SampleSize + 1]);
                    ADJUST_VOLUME_U16(rsmpl, rvlm);
                    ADJUST_VOLUME_U16(lsmpl, lvlm);

                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>(((int32_t)SDL_SwapBE16(*dst16) + lsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768);
                    ++dst16;
                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>(((int32_t)SDL_SwapBE16(*dst16) + rsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768 );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_S32LSB:
            {
                const int32_t *src = (int32_t *)buf->data.data();
                int64_t lsmpl, rsmpl;

                int32_t *dst32 = (int32_t *)dst;
                
                dstlen /= 8;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapLE32(src[pos / SampleSize]);
                    rsmpl = SDL_SwapLE32(src[pos / SampleSize + 1]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst32 = SDL_SwapLE32( CLAMP<int32_t>(((int64_t)SDL_SwapLE32(*dst32) + lsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;
                    *dst32 = SDL_SwapLE32( CLAMP<int32_t>(((int64_t)SDL_SwapLE32(*dst32) + rsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_S32MSB:
            {
                const int32_t *src = (int32_t *)buf->data.data();
                int64_t lsmpl, rsmpl;

                int32_t *dst32 = (int32_t *)dst;
                
                dstlen /= 8;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapBE32(src[pos / SampleSize]);
                    rsmpl = SDL_SwapBE32(src[pos / SampleSize + 1]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst32 = SDL_SwapBE32( CLAMP<int32_t>(((int64_t)SDL_SwapBE32(*dst32) + lsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;
                    *dst32 = SDL_SwapBE32( CLAMP<int32_t>(((int64_t)SDL_SwapBE32(*dst32) + rsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_F32LSB:
            {
                const float *src = (float *)buf->data.data();
                double lsmpl, rsmpl;

                float *fdst32 = (float *)dst;
                
                dstlen /= 8;

                const float fmaxvolume = 1.0f / ((float)SDL_MIX_MAXVOLUME);
                /* !!! FIXME: are these right? */
                const double max_audioval = 3.402823466e+38F;
                const double min_audioval = -3.402823466e+38F;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapFloatLE(src[pos / SampleSize]) * (float)lvlm * fmaxvolume;
                    rsmpl = SDL_SwapFloatLE(src[pos / SampleSize + 1]) * (float)rvlm * fmaxvolume;

                    *fdst32 = SDL_SwapFloatLE( CLAMP<double>(((double)SDL_SwapFloatLE(*fdst32) + lsmpl), min_audioval, max_audioval));
                    ++fdst32;
                    *fdst32 = SDL_SwapFloatLE( CLAMP<double>(((double)SDL_SwapFloatLE(*fdst32) + rsmpl), min_audioval, max_audioval));
                    ++fdst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_F32MSB:
            {
                const float *src = (float *)buf->data.data();
                double lsmpl, rsmpl;

                float *fdst32 = (float *)dst;
                
                dstlen /= 8;

                const float fmaxvolume = 1.0f / ((float)SDL_MIX_MAXVOLUME);
                /* !!! FIXME: are these right? */
                const double max_audioval = 3.402823466e+38F;
                const double min_audioval = -3.402823466e+38F;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = SDL_SwapFloatBE(src[pos / SampleSize]) * (float)lvlm * fmaxvolume;
                    rsmpl = SDL_SwapFloatBE(src[pos / SampleSize + 1]) * (float)rvlm * fmaxvolume;

                    *fdst32 = SDL_SwapFloatBE( CLAMP<double>(((double)SDL_SwapFloatBE(*fdst32) + lsmpl), min_audioval, max_audioval));
                    ++fdst32;
                    *fdst32 = SDL_SwapFloatBE( CLAMP<double>(((double)SDL_SwapFloatBE(*fdst32) + rsmpl), min_audioval, max_audioval));
                    ++fdst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;
            
            default:
                //printf("No mixer for format %x\n", _spec.format);
                break;
        }
    }
    else
    {
        switch(_spec.format)
        {
            case AUDIO_U8:
            {
                const uint8_t *src = (uint8_t *)buf->data.data();
                uint8_t lsmpl, rsmpl;
                
                dstlen /= 2;

                while (dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos);

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = src[ pos ];
                    ADJUST_VOLUME_U8(rsmpl, rvlm);
                    ADJUST_VOLUME_U8(lsmpl, lvlm);

                    *dst = mix8[*dst + lsmpl];
                    ++dst;
                    *dst = mix8[*dst + rsmpl];
                    ++dst;

                    fpos += speedfactor;
                }
                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos);
            }
            break;

            case AUDIO_S8:
            {
                const int8_t *src = (int8_t *)buf->data.data();
                int8_t *dst8 = (int8_t *)dst;
                int8_t lsmpl, rsmpl;
                
                dstlen /= 2;

                while (dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos);

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = src[ pos ];
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst8 = CLAMP<int16_t>((int16_t)*dst8 + (int16_t)lsmpl, SDL_MIN_SINT8, SDL_MAX_SINT8);
                    ++dst8;
                    *dst8 = CLAMP<int16_t>((int16_t)*dst8 + (int16_t)rsmpl, SDL_MIN_SINT8, SDL_MAX_SINT8);
                    ++dst8;
                    
                    fpos += speedfactor;
                }
                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos);
            }
            break;

            case AUDIO_S16LSB:
            {
                const int16_t *src = (int16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                int16_t *dst16 = (int16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapLE16(src[pos / SampleSize]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>((int32_t)SDL_SwapLE16(*dst16) + lsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;
                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>((int32_t)SDL_SwapLE16(*dst16) + rsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_S16MSB:
            {
                const int16_t *src = (int16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                int16_t *dst16 = (int16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapBE16(src[pos / SampleSize]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>((int32_t)SDL_SwapBE16(*dst16) + lsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;
                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>((int32_t)SDL_SwapBE16(*dst16) + rsmpl, SDL_MIN_SINT16, SDL_MAX_SINT16) );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_U16LSB:
            {
                const uint16_t *src = (uint16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                uint16_t *dst16 = (uint16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapLE16(src[pos / SampleSize]);
                    ADJUST_VOLUME_U16(rsmpl, rvlm);
                    ADJUST_VOLUME_U16(lsmpl, lvlm);

                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>(((int32_t)SDL_SwapLE16(*dst16) + lsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768);
                    ++dst16;
                    *dst16 = SDL_SwapLE16( CLAMP<int32_t>(((int32_t)SDL_SwapLE16(*dst16) + rsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768 );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_U16MSB:
            {
                const uint16_t *src = (uint16_t *)buf->data.data();
                int32_t lsmpl, rsmpl;

                uint16_t *dst16 = (uint16_t *)dst;
                
                dstlen /= 4;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapBE16(src[pos / SampleSize]);
                    ADJUST_VOLUME_U16(rsmpl, rvlm);
                    ADJUST_VOLUME_U16(lsmpl, lvlm);

                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>(((int32_t)SDL_SwapBE16(*dst16) + lsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768);
                    ++dst16;
                    *dst16 = SDL_SwapBE16( CLAMP<int32_t>(((int32_t)SDL_SwapBE16(*dst16) + rsmpl) - 32768 * 2,
                                                            SDL_MIN_SINT16, SDL_MAX_SINT16) + 32768 );
                    ++dst16;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_S32LSB:
            {
                const int32_t *src = (int32_t *)buf->data.data();
                int64_t lsmpl, rsmpl;

                int32_t *dst32 = (int32_t *)dst;
                
                dstlen /= 8;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapLE32(src[pos / SampleSize]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst32 = SDL_SwapLE32( CLAMP<int32_t>(((int64_t)SDL_SwapLE32(*dst32) + lsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;
                    *dst32 = SDL_SwapLE32( CLAMP<int32_t>(((int64_t)SDL_SwapLE32(*dst32) + rsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_S32MSB:
            {
                const int32_t *src = (int32_t *)buf->data.data();
                int64_t lsmpl, rsmpl;

                int32_t *dst32 = (int32_t *)dst;
                
                dstlen /= 8;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapBE32(src[pos / SampleSize]);
                    ADJUST_VOLUME(rsmpl, rvlm);
                    ADJUST_VOLUME(lsmpl, lvlm);

                    *dst32 = SDL_SwapBE32( CLAMP<int32_t>(((int64_t)SDL_SwapBE32(*dst32) + lsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;
                    *dst32 = SDL_SwapBE32( CLAMP<int32_t>(((int64_t)SDL_SwapBE32(*dst32) + rsmpl), SDL_MIN_SINT32, SDL_MAX_SINT32));
                    ++dst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_F32LSB:
            {
                const float *src = (float *)buf->data.data();
                double lsmpl, rsmpl;

                float *fdst32 = (float *)dst;
                
                dstlen /= 8;

                const float fmaxvolume = 1.0f / ((float)SDL_MIX_MAXVOLUME);
                /* !!! FIXME: are these right? */
                const double max_audioval = 3.402823466e+38F;
                const double min_audioval = -3.402823466e+38F;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapFloatLE(src[pos / SampleSize]);

                    rsmpl *= (float)rvlm * fmaxvolume;
                    lsmpl *= (float)lvlm * fmaxvolume;

                    *fdst32 = SDL_SwapFloatLE( CLAMP<double>(((double)SDL_SwapFloatLE(*fdst32) + lsmpl), min_audioval, max_audioval));
                    ++fdst32;
                    *fdst32 = SDL_SwapFloatLE( CLAMP<double>(((double)SDL_SwapFloatLE(*fdst32) + rsmpl), min_audioval, max_audioval));
                    ++fdst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;

            case AUDIO_F32MSB:
            {
                const float *src = (float *)buf->data.data();
                double lsmpl, rsmpl;

                float *fdst32 = (float *)dst;
                
                dstlen /= 8;

                const float fmaxvolume = 1.0f / ((float)SDL_MIX_MAXVOLUME);
                /* !!! FIXME: are these right? */
                const double max_audioval = 3.402823466e+38F;
                const double min_audioval = -3.402823466e+38F;

                while(dstlen--)
                {
                    uint32_t pos = stpos + GETFINTPOS(fpos) * SampleSize;

                    if (CheckEnd(chn, &pos, &stpos, &fpos, &endpos, srclen))
                        break;
                    
                    lsmpl = rsmpl = SDL_SwapFloatBE(src[pos / SampleSize]);

                    rsmpl *= (float)rvlm * fmaxvolume;
                    lsmpl *= (float)lvlm * fmaxvolume;

                    *fdst32 = SDL_SwapFloatBE( CLAMP<double>(((double)SDL_SwapFloatBE(*fdst32) + lsmpl), min_audioval, max_audioval));
                    ++fdst32;
                    *fdst32 = SDL_SwapFloatBE( CLAMP<double>(((double)SDL_SwapFloatBE(*fdst32) + rsmpl), min_audioval, max_audioval));
                    ++fdst32;

                    fpos += speedfactor;
                }

                chn->smplpos = GETFRACPOS(fpos);
                chn->pos = stpos + GETFINTPOS(fpos) * SampleSize;
            } 
            break;
            
            default:
                //printf("No mixer for format %x\n", _spec.format);
                break;
        }
    }
}



}